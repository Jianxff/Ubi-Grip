...
def get_config(mano_model_dir: str) -> CN:
    """
    Args:
      mano_model_path (str): Path to MANO model.
    Returns:
      CfgNode: Config as a yacs CfgNode object.
    """
    config_file = str(Path(__file__).absolute().parent / 'config.yaml')
    mano_mean_params = str(Path(__file__).absolute().parent / 'mano_mean_params.npz') 

    cfg = default_config()
    cfg.merge_from_file(config_file)

    cfg.MANO.MODEL_PATH = str(mano_model_dir)
    cfg.MANO.MEAN_PARAMS = mano_mean_params
